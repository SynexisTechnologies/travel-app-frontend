...
import { PrivilegesStepProps } from "../types/props";

const { Text } = Typography;

const PrivilegesStep: React.FC<PrivilegesStepProps> = ({
  isAdminRole,
  privileges,
  setPrivileges,
  role,
  ...
  const getAlertDescription = (role: string) => {
    switch (role) {
      case "super admin":
          ...
          <Row gutter={[16, 16]}>
            {privilegeOptions.map((privilege) => (
              <Col xs={24} sm={12} md={8} key={privilege.key}>
                <Checkbox
                  checked={privileges[privilege.key as keyof typeof privileges]}
                  onChange={(e) =>

...
const UserForm = ({ open, onClose, isEditing, userData, onAdd, onEdit }) => {
  ...
  const getDefaultPrivileges = (role) => {
    ...
    const noPrivileges = Object.keys(allPrivileges).reduce((acc, key) => {
      acc[key] = false;
      return acc;
    }, {});
  ...
  const handlePhoneChange = (phone) => {
    ...
    if (errors.fullPhoneNumber) {
      setErrors((prev) => ({ ...prev, fullPhoneNumber: "" }));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        await onEdit(userData.id, finalFormData);
      } else {

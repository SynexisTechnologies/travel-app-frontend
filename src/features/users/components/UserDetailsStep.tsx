...
import { UserDetailsStepProps } from "../types/props";
...
const UserDetailsStep: React.FC<UserDetailsStepProps> = ({
  ...
  const getPasswordStrengthStatus = (strength: number) => {
    if (strength < 40) return { status: "exception" as const, text: "Weak" };
    if (strength < 60) return { status: "normal" as const, text: "Fair" };
    if (strength < 80) return { status: "active" as const, text: "Good" };
    return { status: "success" as const, text: "Strong" };
  };
